...
**Basis.** Re-graded against `git` HEAD `0693b0fe3a4fdd8a153652dc856466f0e97f3e46` plus
the uncommitted `src/editor/slash.rs`, `blocks.rs`, `commands.rs`, `view.rs` and
`tests/editing.rs` in the working tree at the time of writing. `cargo test` on that
tree: **40/40** UI tests in `tests/editing.rs` and **15/15** unit tests in
`src/editor/mark.rs` pass. Line numbers are valid for that tree.
...
(`view.rs:33-53`); each `Block` (`block.rs:384-401`) is a node-type name +
...
`TextDecoration`/`HighlightStyle` (`view.rs:496-522`, `view.rs:823-866`), with the
text mirror re-diffed after every `InputEvent::Change` (`view.rs:398-451`,
`mark.rs:437-467`) so marks travel with edits. Node types are **data**: a
`BlockSpec` trait (`block.rs:220-301`) supplies label, caps, layout, leading
marker, wrapper chrome, split-into type (now caret-aware, `block.rs:282`), markdown
input rules and slash-menu entries, and a `BlockRegistry` global (`block.rs:307-380`, registrations in
...
| 7 | All 30+ §4 bindings, `Mod` = platform modifier, badges in tooltips/menus | Partial | `actions.rs:61-104` (30 bindings, `secondary-` = platform Mod), now including `secondary-shift-e` emoji, `secondary-shift-2` mention and `secondary-shift-i` insert image (`:73-75`); handlers `keymap.rs:56-205`, new ones at `:183-193` | Still missing: `Mod-Alt-4/5/6`, `Mod-Shift-L/E/R/J` align, `Mod-Shift-D` download, `Mod-Ctrl-L` copy anchor, `Mod-Enter`. **No shortcut badges anywhere** — menus are built with plain `.menu(label, action)` (`gutter.rs:230-249`, `toolbar.rs:132-140`) and tooltips are bare strings. |
| 8 | Enter: split; heading→paragraph at end; empty list item lifts; triple-Enter exits code; Shift/Mod-Enter hard break | Partial | `keymap.rs:209-244`; split `commands.rs:387-432` with `at_end` computed at `:418-419`; `BlockSpec::split_into(attrs, at_end)` (`block.rs:282`) and `Heading::split_into` (`blocks.rs:109-115`) keep a mid-text split a heading and start a paragraph at the end; lift `commands.rs:398-407`; triple-Enter `keymap.rs:227-240`; ArrowDown exit `keymap.rs:321-327`; tests `enter_in_the_middle_of_a_heading_keeps_both_halves_headings`, `enter_splits_a_block_at_the_caret`, `enter_continues_a_list_and_an_empty_item_leaves_it`, `enter_inside_a_code_block_adds_a_line`, `arrow_down_leaves_a_code_block_at_the_end_of_the_document`, `shift_enter_breaks_the_line_inside_a_block` | Only `Mod-Enter` (the second hard-break spelling) is still unbound. |
| 9 | Backspace at block start: join/lift/unwrap/heading-downgrade; whole-atom delete; node-selected delete as its own undo step | Partial | `keymap.rs:246-263`; `commands.rs:435-473` (lift → downgrade → join, deletes a preceding atom node); block-selection delete `selection.rs:99-123` with `Step::Structural` (`history.rs:66-85`); tests `backspace_at_the_start_joins_with_the_previous_block`, `shift_down_selects_whole_blocks_and_backspace_removes_them` | No whole-atom deletion: a mention is ordinary text under a mark, so Backspace removes one character of the name. |
| 10 | Tab/Shift-Tab: list sink/lift, table cells, menu nav with wrap | Partial | `keymap.rs:360-397`; `commands.rs:503-554`; menu wrap `slash.rs:256-266` (`rem_euclid`); test `tab_nests_a_list_item_and_shift_tab_lifts_it` | No table cell traversal (no table node). Sink is capped at "one deeper than the item above" (`commands.rs:511-525`), which matches ProseMirror. |
| 11 | Undo/redo with typing coalescing; `Mod-Y` also redoes | Done | `history.rs:17-134` (500 ms `COALESCE`, `Step::Typing` vs `Structural`); `actions.rs:102-103` binds `secondary-shift-z` and `secondary-y`; `keymap.rs:78-85` captures `Undo`/`Redo` from the inputs; tests `undo_reverts_typing_and_redo_restores_it`, `undo_reverts_a_split`, `undo_reverts_a_node_change`, `redo_through_the_api` | — |
| 12 | Slash opens on `/` at start or after whitespace; also `Mod-/` and gutter `+` | Done | `slash.rs:128-190` (word-boundary check at `:172-175`); `Mod-/` → `actions.rs:72` → `slash.rs:85-119` (`open_suggestion` is now the generic opener for `/`, `@` and `:`); `+` button `gutter.rs:88-93` → `gutter.rs:119-124`; tests `the_slash_menu_filters_and_runs_an_item`, `the_gutter_plus_adds_a_block_below`, `a_colon_in_prose_does_not_hold_a_menu_open` | — |
...
| 26 | Image upload node: dashed drop zone, exact strings, per-file progress, drag states, accent border when selected | Partial | `blocks.rs:715-766`: dashed 120 px zone with the `image-up` icon and the literal `Click to upload or drag and drop`, hover fill, accent border while the node is selected, `on_click` → `NotionEditor::pick_image` (`commands.rs:669-687`, `cx.prompt_for_paths`) and `on_drop::<ExternalPaths>` → `set_image_source` (`commands.rs:690-706`, records `Step::Structural`); a placed image gets a 2 px primary outline when selected (`blocks.rs:757-765`); test `an_image_block_takes_a_dropped_file` | No per-file progress rows, no `limit`/`maxSize` subtext or enforcement, no MIME filtering, no separate `imageUpload` node, no caption child, and no distinct `drag-active`/`drag-over` washes. The "Click to upload" half is not italicised. |
...
| 29 | Placeholder only in the focused empty block, italic `Write, type '/' for commands…` | Partial | exact string `blocks.rs:51-53`; focus rule `view.rs:143-165` + `view.rs:124-137`; the input shows it only when empty | Not italic (`view.rs:684-713` sets no per-placeholder font style). `Heading` sets `placeholder_always` (`blocks.rs:105-107`), so every heading shows `Heading N` whether focused or not — a deliberate Notion-ism, not the template's rule. Other specs define their own strings (`blocks.rs:189, 252, 368, 475, 801, 871`), shown under the focus rule. |
| 30 | A trailing paragraph always exists after a non-paragraph last node; clicking below focuses it | Partial | `commands.rs:365-381` `ensure_paragraph_after`, called after HR (`:323`), image (`:360`), code-block exit (`keymap.rs:235`), ArrowDown off the last block (`keymap.rs:321-327`) and non-textual input rules (`input_rules.rs:189`); click-below `view.rs:915-929` + `:940-958`; tests `clicking_below_the_document_appends_a_paragraph`, `arrow_down_leaves_a_code_block_at_the_end_of_the_document` | It is a set of call sites, not an invariant: deleting the paragraph after a rule/image leaves a non-textual node last until you arrow down or click below. Bottom space is a fixed `280 px` (`style.rs:18`), not `30vh`. |
| 31 | Node selection highlight = translucent brand fill, 8 px radius; images get a 2 px outline | Partial | `view.rs:771-773` `.rounded(px(4.)).bg(theme.selection.opacity(0.4))`; images do get their own 2 px outline in `theme.primary` (`blocks.rs:757-765`) and the empty drop zone an accent border (`blocks.rs:730-734`) | 4 px radius, not 8; the fill and the image outline use the theme's selection/primary colors, not brand-500 at 20 %. |
| 32 | Selection stays visible when focus moves to a toolbar/popover | Done | The platform does it: the input element paints its selection paths whenever the **window** is active, with no focus condition (`gpui-kit/crates/base/src/input/base/element.rs:2280-2296`), and `on_blur` deliberately keeps the selection (`state.rs:3156-3157`, "NOTE: Do not cancel select, when blur."). The range itself lives in the block's `EditorState`, which is why toolbar and menu commands still act on the right text after focus moves (`toolbar.rs:32-45`, `view.rs:226-230`); the link editor takes focus into its own input (`toolbar.rs:242-245`) and the selection stays painted. | (Comes free from gpui-kit, not from editor code; the fill is the theme's selection color — see #39.) |
| 33 | Content column 708 px, side gutters ≥96 px, padding 48/48/30vh | Partial | `style.rs:10` `PAGE_WIDTH = 708`, `:12` `PAGE_PADDING = 48`; applied `view.rs:910-913`; wrap width 612 px (`view.rs:63`) | `GUTTER_WIDTH = 96` (`style.rs:14`) is declared and **never used** — the column is simply centred, so side margins are whatever the window leaves and can fall below 96 px. Bottom space is a fixed 280 px, not 30vh. No ≤768 px/≤480 px behaviour. |
| 34 | Paragraph 16/1.6/400 + 20 px top; H1 1.5em/700/3em-top, H2 1.25em/700/2.5em, H3 1.125em/600/2em; first heading no top margin | Done | `style.rs:21-25` (16 px, 1.6, 20 px paragraph top margin); `blocks.rs:80-99` — 24 px/700, 20 px/700, 18 px/600 with `margin_em` factors `3.0/2.5/2.0` of each heading's own size (`:85-90`), i.e. 72/50/36 px; H4–H6 fall back to the spec's H4 values (16 px/600/2em); first block margin 0 at `view.rs:747-753` | — |
| 35 | Lists: 1.5em vertical margin, 1.5em left padding, marker cycles, 24 px indent | Partial | marker cycles `blocks.rs:197-212` (disc/circle/square) and `:260-276` (decimal/alpha/roman, `alpha_ordinal`/`roman_ordinal` `:306-342`); `style.rs:28` `INDENT_WIDTH = 24`; `list_layout()` now `margin_top: 24px` (= 1.5em) with `leading_width: 24px` (`blocks.rs:159-168`) | Top margin and left gutter now match; there is still no explicit 1.5em bottom margin (spacing below a list comes from the next block's own top margin), and consecutive items get a 2 px gap (`view.rs:747-753`) rather than CSS-collapsed zero. |
| 36 | Blockquote: 0.25em bar in gray-900, 1em left padding, 1.5rem vertical margin | Partial | `blocks.rs:479-493` — 4 px bar in `theme.foreground`, `pl(16px)`, `py(6px)`; `margin_top: 24px` (`:465-473`) | `margin_bottom: 0` instead of 1.5rem, and the bar is rounded (`rounded(px(2.))`) where the template's is square. |
| 37 | Code block 1em padding, 1 px border, 6 px radius, mono 16 px; inline code 0.875em, tinted bg + border | Partial | block: `blocks.rs:548-591` (`p(16)`, `border_1`, `rounded(6)`, `mono: true`, hover language menu); inline: `view.rs:842-845` (background + red foreground, `style.rs:55-69`) | Code-block text is 14 px, not 16 px. Inline code has no border and cannot be 0.875em — see §4. |
| 38 | Task checkbox 1em square, 0.25rem radius, checked = gray-a-900 fill + white check, checked text 50 % + strike | Partial | `blocks.rs:376-412` (16 px box, `rounded(4)`, click toggles) and `:359-366` (`text_opacity 0.5` + `strikethrough`), painted through the decoration layer (`view.rs:500-502`) | Fill/border use `theme.primary`, not gray-a-900; the check is the text glyph `✓` at 11 px rather than a masked SVG; no 80 ms transition. |
...
**Counts — Done 8 · Partial 33 · Missing 1 · Not possible on this stack 0.**

Done: #4, #11, #12, #16, #19, #28, **#32**, **#34**. Missing: **#27** (table) only.
Since the previous grading, #32 moved Missing → Done (platform evidence, not new
code), #34 Partial → Done, #26 Missing → Partial, and #7/#8/#14/#24/#31/#35/#40
narrowed while staying Partial.
...
1. **Table node (#27, and the Tab half of #10).** The only wholly missing item. A
   `Table` `BlockSpec` whose `render_body` lays out a grid of child `EditorState`s
   held in `BlockAttrs.extra` or a side table keyed by `BlockId`; Tab/Shift-Tab
   handled in `keymap.rs:360-397` before the list branch. Also adds the Table entry
   the slash menu still lacks (#14).
2. **Shortcut badges everywhere (#7, #24, #42).** Add `shortcut: Option<&str>` to
   `SlashItem` and use gpui-kit's label+badge menu row for `gutter.rs:209-250` and
   `toolbar.rs:132-140`; render the same string as a `kbd` pill in tooltips.
3. **Image upload completeness (#26).** On top of the picker and drop handler,
   ...
   caption child; `commands.rs:669-706` is the place they hook in.
4. **Mark input rules on paste (#5).** In `apply_markdown_prefix`
...
5. **Ghost `Filter...` decoration and item availability (#13).** Render
...
6. **Home/End and scroll-into-view in menus (#15).** Capture `MoveToStartOfLine`/
...
7. **Recent colors and 10 highlights (#24).** Extend `HighlightColor::ALL`
   (`mark.rs:73`) to the template's 10 names and keep a `recent: Vec<ApplyColor>` on
   the editor, rendered as the first group of `color_menu` (`gutter.rs:265-277`);
   reuse `toolbar.rs:369-374`'s `swatch_row` so both menus show swatches.
8. **Palette and chrome tokens (#39, #40, #41, #31).** Replace the ad-hoc values in
   `style.rs:76-123` with the §5.4 table (they are currently off, and orange is
   swapped light/dark), and add a brand ramp so selection, caret, link and active
   icons stop inheriting the kit theme.
9. **Drag affordances (#21, #23).** `cursor_grab()` on the handle, a `dragging: bool`
   ...
   `block.rows` (`block.rs:399`) for the >40 px case.
10. **Trailing-node invariant (#30).** Make "last block is textual" an invariant
...
11. **Link popover completeness (#25) and the two-line grip tooltip (#22).** Add the
    open-in-new-tab button and the "empty field but active link" enable case in
    `toolbar.rs:296-357`; render the grip tooltip as two stacked lines.
12. **Remaining bindings (#7, #8).** Bind `Mod-Enter` as a second hard break,
...
* **Marks are decorations, not styled spans; per-range font size/family is
  ...
  nothing (`view.rs:862`); inline `code` cannot be 0.875em or use a mono family
  inside prose, so it is shown as a tinted background plus Notion's red foreground
  (`view.rs:842-845`, `style.rs:55-69`); a completed to-do is struck through by
  synthesising a full-width `Strike` run (`view.rs:500-502`) rather than styling the
  text element.
* **One `EditorState` per block instead of one document buffer.** The same limit
  ...
  (`view.rs:684-713`, created in `view.rs:244-275`). This is why the editor re-implements cross-block caret
  movement (`keymap.rs:284-358`), cross-block selection (`selection.rs`) and a
...
* **No text alignment.** `InputState::set_text_align` is documented
  ...
  soft-wrapping multi-line editor (`view.rs:268`), so the template's four align
...
* **Mentions and emoji are not inline atoms.** There is no inline-embed/atomic-widget
  ...
  `view.rs:858-861`) and an emoji is the literal character (`slash.rs:288-298`);
...
* **Block text is a mirror plus a diff, not an edit stream.** The input reports only
  ...
  (`view.rs:398-451`, `mark.rs:432-467`) to remap marks — and infers a paste from
  "an insertion longer than one character containing a newline" (`view.rs:410-414`)
...
* **Heights are estimated before layout.** The input lays out inside the height it is
  ...
  (`view.rs:612-665`; `remeasure` at `view.rs:615-624` runs only when a block's text,
  type or nesting changes), with the row count cached on the block (`block.rs:397-399`).
